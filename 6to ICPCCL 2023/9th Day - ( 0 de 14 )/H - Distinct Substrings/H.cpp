...
struct SuffixAutomaton{
  struct state {
    // link es el estado al que debe volver para seguir estando en el substring mÃ¡s largo.
    // link serian los arcos morados.

    // len es el largo del sufijo mas largo del que el nodo se encarga. (camino mas largo)
    int len, link;
...
int main(){
  ...
  while(n --){
    ...
    for (int i = 0; i < SA.sz; i ++){
      ...
      if (SA.st[i].len <= S.size()) {
        cant += SA.st[i].len - max(k - 1, SA.st[SA.st[i].link].len + 1) + 1;
      }
    }
    cout << cant << '\n';
  }
